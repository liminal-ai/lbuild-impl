...
| BUG-WIN-006 | Local patch | **Not reproduced; likely structurally unreachable from `story-orchestrate resume` alone.** A second pass on 2026-05-07 ran `story-orchestrate resume` after writing a ruling-approval artifact for the `needs-ruling` terminal: ruling was ingested cleanly (sequence 22), planner spawned fresh (sequence 23), and emitted a fresh terminal in ~24 s. Both `story-lead-provider-started` events on the resume path explicitly recorded "Fresh story-lead provider turn executed without planner session resume" — the orchestrator carries planner context across turns via the stdin-delivered prompt + durable state, **not** via codex's own session-resume protocol. To actually exercise BUG-WIN-006's code path on Windows v0.4.0 requires interrupting **mid-`run-implement` or mid-`run-verify`** (so the child operation persists a `sessionId`) and then resuming so the orchestrator reopens that child via `codex exec resume <sessionId>`. This run never re-entered those paths because story-lead jumped straight from `ruling-received` → `accept-story`. _Pending mid-child interrupt test._ |
